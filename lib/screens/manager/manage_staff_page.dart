...
class _ManageStaffPageState extends State<ManageStaffPage> {
  ...
  Future<void> loadStaff() async {
    ...
    try {
      ...
      list.sort((a, b) {
        ...
        return getFullName(a).toLowerCase().compareTo(getFullName(b).toLowerCase());
      });
  ...
  List<Map<String, dynamic>> getFilteredStaff() {
    ...
    if (selectedRole != 'All') {
      filtered = filtered.where((staff) => getRole(staff) == selectedRole.toLowerCase()).toList();
    }
  ...
  int get inactiveStaff => staffList.where((staff) => !isActiveStaff(staff)).length;
  int get managerCount => staffList.where((staff) => getRole(staff) == 'manager').length;
  int get supervisorCount => staffList.where((staff) => getRole(staff) == 'supervisor').length;

  Future<void> updateStaffStatus(Map<String, dynamic> staff, bool active) async {
    if (isCurrentUser(staff)) {
      showMessage('You cannot deactivate or reactivate your own manager account from Manage Staff.', isError: true);
      return;
    }
    ...
    try {
      ...
      showMessage(active ? 'Account activated successfully.' : 'Account deactivated successfully.');
      await loadStaff();
    } catch (e) {
  ...
  Future<bool> confirmStatusChange(Map<String, dynamic> staff, bool active) async {
    if (isCurrentUser(staff)) {
      showMessage('This is your current account. Action blocked for safety.', isError: true);
      return false;
    }
  ...
  Future<void> showEditStaffSheet(Map<String, dynamic> staff) async {
    ...
    final fullNameController = TextEditingController(text: getFullName(staff));
    final usernameController = TextEditingController(text: getUsername(staff));
    final emailController = TextEditingController(text: getEmail(staff) == '-' ? '' : getEmail(staff));
    final phoneController = TextEditingController(text: getPhone(staff) == '-' ? '' : getPhone(staff));
    final branchController = TextEditingController(text: getBranch(staff) == '-' ? '' : getBranch(staff));
    final departmentController = TextEditingController(text: getDepartment(staff) == '-' ? '' : getDepartment(staff));
                    ...
                    children: [
                      Center(
                        child: CircleAvatar(
                          ...
                          child: avatarUrl.isEmpty ? const Icon(Icons.person, color: mulberry, size: 40) : null,
                        ),
                      ),
                      const SizedBox(height: 12),
                      Center(
                        child: Text(
                          isSelf ? 'Edit My Manager Profile' : 'Edit Staff Profile',
                          style: const TextStyle(color: mulberryDark, fontFamily: 'Georgia', fontSize: 22, fontWeight: FontWeight.bold),
                        ),
                      ),
                      if (isSelf) ...[
                      ...
                      buildTextField(controller: fullNameController, label: 'Full Name', icon: Icons.badge),
                      const SizedBox(height: 12),
                      buildTextField(controller: usernameController, label: 'Username', icon: Icons.alternate_email),
                      const SizedBox(height: 12),
                      buildTextField(controller: emailController, label: 'Email', icon: Icons.email, keyboardType: TextInputType.emailAddress),
                      const SizedBox(height: 12),
                      buildTextField(controller: phoneController, label: 'Phone', icon: Icons.phone, keyboardType: TextInputType.phone),
                      const SizedBox(height: 12),
                      buildTextField(controller: branchController, label: 'Branch', icon: Icons.store),
                      const SizedBox(height: 12),
                      buildTextField(controller: departmentController, label: 'Department', icon: Icons.apartment),
                      const SizedBox(height: 12),
                      if (isSelf)
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          decoration: buildInputDecoration(label: 'Role', icon: Icons.admin_panel_settings),
                          items: const [
                        ...
                        child: ElevatedButton.icon(
                          onPressed: () async {
                            ...
                            if (fullName.isEmpty || username.isEmpty) {
                              showMessage('Full name and username are required.', isError: true);
                              return;
                            }
                            ...
                            try {
                              await supabase.from('profiles').update({
                                ...
                                'branch': branchController.text.trim().isEmpty ? null : branchController.text.trim(),
  ...
  Widget buildSelfProtectionNotice() {
    return Container(
      ...
      child: Text(
        ...
        style: TextStyle(color: Colors.orange.shade900, fontWeight: FontWeight.w600, height: 1.3),
      ),
    );
  }
              ...
              children: [
                Center(
                  child: CircleAvatar(
                    ...
                    backgroundColor: softWhite,
                    backgroundImage: avatarUrl.isNotEmpty ? NetworkImage(avatarUrl) : null,
                    child: avatarUrl.isEmpty ? const Icon(Icons.person, color: mulberry, size: 45) : null,
                  ),
                ),
                const SizedBox(height: 12),
                Center(
                  child: Text(
                    ...
                    style: const TextStyle(color: mulberryDark, fontFamily: 'Georgia', fontSize: 23, fontWeight: FontWeight.bold),
                  ),
                ),
                const SizedBox(height: 10),
                Center(
                  child: Wrap(
                    ...
                    runSpacing: 8,
                    children: [
                      buildStatusBadge(active ? 'ACTIVE' : 'INACTIVE', active ? Colors.green : Colors.red),
                      buildStatusBadge(formatRole(role).toUpperCase(), getRoleColor(role)),
                      if (isSelf) buildStatusBadge('CURRENT ACCOUNT', Colors.orange),
                    ],
                  ),
                ),
                const SizedBox(height: 18),
                buildDetailRow(Icons.badge, 'Profile ID', getProfileId(staff)),
                buildDetailRow(Icons.alternate_email, 'Username', getUsername(staff)),
                ...
                buildDetailRow(Icons.apartment, 'Department', getDepartment(staff)),
                if (isSelf) ...[
                  const SizedBox(height: 10),
                  buildSelfProtectionNotice(),
                ],
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: isSelf
                            ...
                            : () async {
                                Navigator.pop(bottomSheetContext);
                                final confirm = await confirmStatusChange(staff, !active);
                                if (confirm) {
  ...
  Widget buildStatusBadge(String text, Color color) {
    return Container(
      ...
      child: Text(text, style: TextStyle(color: color, fontSize: 11.5, fontWeight: FontWeight.bold)),
    );
  }
  ...
  Widget buildHeader() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(colors: [mulberryDark, mulberry, mulberryLight], begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(22),
        boxShadow: [BoxShadow(color: mulberryDark.withOpacity(0.16), blurRadius: 16, offset: const Offset(0, 7))],
      ),
      child: const Row(
        children: [
          CircleAvatar(radius: 27, backgroundColor: Color(0x33F5ECD7), child: Icon(Icons.people, color: cream, size: 30)),
          SizedBox(width: 14),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Manage Staff', style: TextStyle(color: cream, fontFamily: 'Georgia', fontSize: 21, fontWeight: FontWeight.bold)),
                SizedBox(height: 5),
                Text('Manage staff profile, role and account status.', style: TextStyle(color: creamDark, fontWeight: FontWeight.w500)),
              ],
            ),
          ),
        ],
  ...
  Widget buildSummaryCards() {
    return GridView.count(
      ...
      children: [
        buildSummaryCard('Total', totalStaff.toString(), Icons.people, mulberry),
        buildSummaryCard('Active', activeStaff.toString(), Icons.check_circle, Colors.green),
        buildSummaryCard('Inactive', inactiveStaff.toString(), Icons.block, Colors.red),
        buildSummaryCard('Supervisor', supervisorCount.toString(), Icons.admin_panel_settings, Colors.blue),
      ],
    );
  }
  ...
  Widget buildSummaryCard(String title, String value, IconData icon, Color color) {
    return Container(
      ...
      child: Row(
        children: [
          CircleAvatar(radius: 18, backgroundColor: color.withOpacity(0.12), child: Icon(icon, color: color, size: 19)),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(value, style: const TextStyle(color: mulberryDark, fontWeight: FontWeight.bold, fontSize: 18)),
                Text(title, style: TextStyle(color: Colors.grey.shade600, fontSize: 11.5, fontWeight: FontWeight.w600)),
              ],
            ),
          ),
        ],
      ...
      children: [
        TextField(
          ...
          decoration: InputDecoration(
            ...
            filled: true,
            fillColor: softWhite,
            enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: creamDark.withOpacity(0.85))),
            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: const BorderSide(color: mulberry, width: 1.8)),
          ),
        ),
        const SizedBox(height: 10),
        Row(
          children: [
            Expanded(
              child: buildDropdown(
                ...
                onChanged: (value) => setState(() => selectedRole = value ?? 'All'),
              ),
            ),
            const SizedBox(width: 10),
            Expanded(
  ...
  Widget buildDropdown({required String value, required List<String> items, required ValueChanged<String?> onChanged}) {
    return DropdownButtonFormField<String>(
      ...
      decoration: InputDecoration(
        ...
        enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide(color: creamDark.withOpacity(0.85))),
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: const BorderSide(color: mulberry)),
      ),
  ...
  Widget buildStaffCard(Map<String, dynamic> staff) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isSelf ? Colors.orange.shade200 : creamDark.withOpacity(0.75)),
          ...
          child: Row(
            children: [
              CircleAvatar(
                ...
                backgroundColor: cream,
                backgroundImage: avatarUrl.isNotEmpty ? NetworkImage(avatarUrl) : null,
                child: avatarUrl.isEmpty ? const Icon(Icons.person, color: mulberry) : null,
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                      ...
                      children: [
                        Expanded(
                          child: Text(
                            ...
                            style: const TextStyle(color: mulberryDark, fontWeight: FontWeight.bold, fontSize: 15.5),
                          ),
                        ),
                    ...
                    Text('@${getUsername(staff)}', style: TextStyle(color: Colors.grey.shade600, fontSize: 12.5)),
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (context) => [
                  ...
                  if (!isSelf)
                    PopupMenuItem(
                      value: 'toggle',
                      child: Text(active ? 'Deactivate' : 'Activate', style: TextStyle(color: active ? Colors.red : Colors.green)),
                    ),
                ],
              ),
            ],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: widget.showAppBar
          ? AppBar(
              title: const Text('Manage Staff', style: TextStyle(color: cream, fontWeight: FontWeight.bold)),
              centerTitle: true,
              backgroundColor: mulberry,
              foregroundColor: cream,
              elevation: 0,
              actions: [IconButton(onPressed: loadStaff, icon: const Icon(Icons.refresh))],
            )
          : null,
      body: isLoading
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  if (filteredStaff.isEmpty) buildEmptyState() else ...filteredStaff.map(buildStaffCard),
                ],
              ),
            ),
    );
